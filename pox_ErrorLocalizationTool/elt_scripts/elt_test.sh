...
log_pid=$!
run_term $terminal "./pox.py $deb --flow_table_controller=config/flow_table_config.cfg ext.debugger.controllers.router ext.debugger.controllers.server ext.debugger.controllers.monitor py log.level --WARNING"
#" --flow_table_controller=config/flow_table_config.cfg forwarding.l2_learning ext.debugger.controllers.interrupter"
pox_pid=$!
run_term $terminal "mn --topo=elt_test --custom `pwd`/ext/debugger/topology/elt_test_topo.py --controller=remote"
mn_pid=$!
